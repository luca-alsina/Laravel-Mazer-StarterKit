<div class="modal fade text-left w-100" id="modal-category-update-{{ $category->id }}" tabindex="-1" role="dialog"
     aria-labelledby="modal-user-create" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl"
         role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modal-user-create">{{ __('admin.pages.categories.edit') }} </h4>
            ...
            <form method="post" action="{{ route('admin.categories.update', compact('category')) }}">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="title" id="title" value="{{ $category->title }}" placeholder="{{ __('admin.pages.categories.index.name') }}">
                    </div>
{{--                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control form-select" name="parent_id" id="parent_id">
                            ...
                            @foreach($categories as $sCategory)
                                <option value="{{ $sCategory->id }}" @if(!empty($category->parent) && $category->parent->id === $sCategory->id) selected @endif>{{ $sCategory->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="description">{{ __('admin.pages.categories.index.description') }}</label>
                        <textarea rows="5" class="form-control" name="description" id="description" placeholder="{{ __('admin.pages.categories.index.description') }}">{{ $category->description }}</textarea>
                    </div>
                </div>
